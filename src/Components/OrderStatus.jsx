  ...
  return (
    ...
    <div className="flex flex-col items-center justify-center min-h-screen ">
      <div className="w-full max-w-3xl bg-white p-8 rounded-lg shadow-xl">
        <h2 className="text-3xl font-bold text-center text-[#AD6237]"> Thank You for Your Order! </h2>
        ...
        <div className="mt-6 border rounded-lg p-4 bg-gray-50 shadow-sm">
          ...
          <p className="text-gray-700">Expected Delivery: <span className="text-[#AD6237] font-semibold">{deliveryDate.toDateString()}</span></p>
        </div>

        <div className="mt-6 flex flex-col md:flex-row items-center gap-6">
          <img src={product.image} alt="Product" className="w-40 h-40 rounded-lg object-contain shadow-md" />
          <div className="text-center md:text-left">
            <h3 className="text-xl font-semibold">{product.title}</h3>
            <p className="text-gray-600 mt-2">SKU: {order.sku}</p>
            <p className="text-gray-600 mt-2">Price: <span className="text-[#AD6237] font-bold">${product.price}</span></p>
          </div>
        </div>
